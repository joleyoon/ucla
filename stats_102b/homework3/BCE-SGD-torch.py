...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.out_activation(self.fc3(x))
        return x


def hidden_combinations():
    for h1, h2 in product(hidden_layers[0], hidden_layers[1]):
        yield h1, h2
...
for batch_size in batch_sizes:
    for h1, h2 in hidden_combinations():
        print(f"Trying model with h1={h1}, h2={h2}, batch={batch_size}")
        model = MLP(p, h1, h2)
...
train_fig = Path(__file__).with_name("BCE-SGD-torch_training.png")
val_fig = Path(__file__).with_name("BCE-SGD-torch_validation.png")
